...
def compute_subscription_breakdown(df: pd.DataFrame, today: date | None = None) -> dict:
    ...
    return {
        ...
        "redundancy_waste": redundancy_waste,
    }
...
def compute_lifestyle_insights(
    df: pd.DataFrame, kpis: dict, fixed_costs: dict, sub_breakdown: dict, today: date | None = None
) -> list:
    ...
    if phone_waste > 0:
        insights.append(
            f"You have multiple active phone plans — cancelling the cheapest saves ~${phone_waste * 12:.0f}/yr"
        )
    ...
    if avg_income > 0 and rent > 0:
        ...
        if rent_pct > 0.33:
            ...
            insights.append(
                f"Rent is {rent_pct * 100:.0f}% of avg income — the 30% rule suggests ${target:,.0f}/mo"
            )
    if not df.empty:
        ...
        if dubai_avg > 300:
            insights.append(
                f"International spending averages ${dubai_avg:.0f}/mo — CSP has no FX fee, saving ~${dubai_avg * 0.03 * 12:.0f}/yr vs 3% foreign fee cards"
            )
        ...
        if food_avg > 500:
            insights.append(
                f"Dining out costs ${food_avg:.0f}/mo — cooking at home 2 extra nights/week could save ~${food_avg * 0.20:.0f}/mo"
            )
    total_subs = sub_breakdown.get("total_monthly", 0.0)
    if total_subs > 150:
        insights.append(
            f"Subscriptions total ${total_subs:.0f}/mo (${total_subs * 12:.0f}/yr) — review for unused services"
        )
    if not insights:
...
def compute_card_csp_analysis(df: pd.DataFrame, card: dict, today: date | None = None) -> dict:
    ...
    empty = {
        "net_annual_value": -annual_fee,
        "gross_rewards": 0.0,
        "annual_fee": annual_fee,
        "by_category": [],
        "top_opportunity": "",
        "fx_note": "CSP has no foreign transaction fees — always use it for international purchases",
    }
    if df.empty:
        return empty
    expense_df = df[df["amount"] < 0].copy()
    ...
    for cat, annual_spend in sorted(spending.items(), key=lambda x: x[1], reverse=True):
        ...
        by_category.append(
            {
                "category": cat,
                "monthly_spend": round(annual_spend / 12, 2),
                "earn_rate": f"{earn_rate:.0f}x",
...
def build_context(
    ...
    lifestyle_insights = compute_lifestyle_insights(
        df, kpis, fixed_costs, sub_breakdown, today=today
    )
    ...
    csp_card = next(
        (c for c in (cards or {}).get("cards", []) if "sapphire" in c["name"].lower()), None
    )
...
def compute_health_score(
    ...
    if income == 0:
        d1 = 10
        st1 = "warn"
        ex1 = "No income recorded this month — check statement coverage"
    elif expenses <= income:
        d1 = 25
        st1 = "pass"
        ex1 = f"Expenses (${expenses:,.0f}) are below income (${income:,.0f}) — positive cash flow"
    else:
        ratio = (expenses - income) / income
        d1 = max(0, round(25 - ratio * 50))
        st1 = "fail" if d1 == 0 else "warn"
        ex1 = f"Expenses exceeded income by {ratio * 100:.0f}% this month"
    dimensions.append(
        {"label": "Income coverage", "score": d1, "max": 25, "status": st1, "explanation": ex1}
    )
    total += d1
    if st1 == "pass":
        passing.append("Income coverage")
    elif d1 == 0:
        failing.append("Income coverage")

    # Dim 2: Savings rate (20 pts)
    if income <= 0:
        d2 = 0
        st2 = "fail"
        ex2 = "No income this month — savings rate cannot be calculated"
    else:
        sr = kpis["savings_rate"]
        d2 = round(min(sr / 0.20, 1.0) * 20) if sr > 0 else 0
        if d2 >= 18:
            st2 = "pass"
            ex2 = f"Savings rate {sr * 100:.1f}% — on track for the 20% target"
        elif d2 >= 8:
            st2 = "warn"
            ex2 = f"Savings rate {sr * 100:.1f}% — aim for 20% to build wealth"
        else:
            st2 = "fail"
            ex2 = f"Savings rate {sr * 100:.1f}% — expenses are consuming most of income"
    dimensions.append(
        {"label": "Savings rate", "score": d2, "max": 20, "status": st2, "explanation": ex2}
    )
    total += d2
    if st2 == "pass":
    ...
    liquid = sum(
        ...
        if a.get("type") in ["checking", "savings"] and a["balance"] > 0
    )
    if liquid == 0:
        d3 = 3
        st3 = "warn"
        ex3 = "No liquid assets tracked — add checking/savings accounts"
    else:
        dr = credit_bal / liquid
        if dr < 0.10:
            d3 = 15
            st3 = "pass"
            ex3 = f"Credit balance ${credit_bal:,.0f} is only {dr * 100:.0f}% of liquid savings"
        elif dr < 0.25:
            d3 = 8
            st3 = "warn"
            ex3 = f"Credit balance ${credit_bal:,.0f} is {dr * 100:.0f}% of liquid savings — aim to pay it down"
        elif dr < 0.50:
            d3 = 3
            st3 = "warn"
            ex3 = f"Credit balance ${credit_bal:,.0f} is {dr * 100:.0f}% of liquid savings — high debt burden"
        else:
            d3 = 0
            st3 = "fail"
            ex3 = f"Credit balance ${credit_bal:,.0f} exceeds 50% of liquid savings — prioritize payoff"
    dimensions.append(
        {"label": "Debt burden", "score": d3, "max": 15, "status": st3, "explanation": ex3}
    )
    total += d3
    if st3 == "pass":
        passing.append("Debt burden")
    elif d3 == 0:
        failing.append("Debt burden")
    ...
    if fixed["avg_income"] == 0:
        d4 = 8
        st4 = "warn"
        ex4 = "Avg income unclear — fixed cost ratio cannot be calculated"
    elif pct < 0.40:
        d4 = 15
        st4 = "pass"
        ex4 = f"Fixed costs (rent, phone, insurance) are {pct * 100:.0f}% of income — healthy"
    elif pct < 0.55:
        d4 = 8
        st4 = "warn"
        ex4 = f"Fixed costs are {pct * 100:.0f}% of income — leaves little room for savings"
    else:
        d4 = 0
        st4 = "fail"
        ex4 = f"Fixed costs are {pct * 100:.0f}% of income — above 55% is a financial stress zone"
    dimensions.append(
        {"label": "Fixed cost ratio", "score": d4, "max": 15, "status": st4, "explanation": ex4}
    )
    total += d4
    if st4 == "pass":
        passing.append("Fixed cost ratio")
    elif d4 == 0:
        failing.append("Fixed cost ratio")

    # Dim 5: Emergency fund (15 pts)
    if expenses == 0:
        d5 = 15
        st5 = "pass"
        ex5 = "No expense data — emergency fund assumed adequate"
    else:
        ratio5 = liquid / (expenses * 3)
        if ratio5 >= 1.0:
            d5 = 15
            st5 = "pass"
            ex5 = f"Liquid savings cover {ratio5:.1f}x three months of expenses — excellent buffer"
        elif ratio5 >= 0.33:
            d5 = 8
            st5 = "warn"
            ex5 = f"Liquid savings cover {ratio5 * 3:.1f} months of expenses — target is 3 months"
        else:
            d5 = 0
            st5 = "fail"
            ex5 = "Liquid savings cover less than 1 month of expenses — build an emergency fund"
    dimensions.append(
        {"label": "Emergency fund", "score": d5, "max": 15, "status": st5, "explanation": ex5}
    )
    total += d5
    if st5 == "pass":
    ...
    if income <= 0:
        d6 = 5
        st6 = "warn"
        ex6 = "No income data — investment rate unknown"
    elif inv_amt == 0:
        d6 = 0
        st6 = "fail"
        ex6 = "No investments recorded this month — consider contributing to a brokerage"
    else:
        ir = inv_amt / income
        if ir >= 0.05:
            d6 = 10
            st6 = "pass"
            ex6 = f"Investing {ir * 100:.0f}% of income this month — great habit"
        else:
            d6 = 5
            st6 = "warn"
            ex6 = f"Investing {ir * 100:.0f}% of income — aim for 5%+ to grow wealth"
    dimensions.append(
        {"label": "Investment rate", "score": d6, "max": 10, "status": st6, "explanation": ex6}
    )
    total += d6
    if st6 == "pass":
        passing.append("Investment rate")
    elif d6 == 0:
        failing.append("Investment rate")
...
def compute_actionable_cuts(df: pd.DataFrame, category_trends: list) -> list:
    ...
    for trend in category_trends:
        ...
        trend["prior_amounts"] + [trend["current_amount"]]
        avg = (
            sum(trend["prior_amounts"]) / len(trend["prior_amounts"])
...
def compute_spending_pct_of_income(
    df: pd.DataFrame, income: float, today: date | None = None
) -> list:
    ...
    return [
        {
            ...
            "pct_of_income": round(float(amt) / income * 100, 1),
        }
    ...
    return [
        {
            ...
            "share_of_total": round(a["balance"] / positive_total, 4)
            if positive_total > 0 and a["balance"] > 0
            else 0.0,
        }
        for a in accts
    ]
...
def compute_top_merchants(df: pd.DataFrame, month: str) -> list:
    ...
    grouped = (
        ...
        .agg(
            amount=("amount", lambda x: abs(x.sum())),
...
def _get_at_risk_goals(goals: dict, today: date | None = None) -> list:
    ...
    for g in goals.get("goals", []):
        try:
            ...
            total_months = max(
                1, (deadline.year - created.year) * 12 + (deadline.month - created.month)
            )
            elapsed_months = max(
                0, (today.year - created.year) * 12 + (today.month - created.month)
            )
