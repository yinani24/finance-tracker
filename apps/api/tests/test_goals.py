...
class TestGoalAPI:
    ...
    def test_create_goal(self, client: TestClient, db_session: Session):
        payload = {
            "name": "Emergency Fund", "goal_type": "savings", "target_amount": 10000.00,
            "current_amount": 2500.00, "is_monthly": False,
        }
    ...
    def test_create_monthly_goal(self, client: TestClient, db_session: Session):
        payload = {
            "name": "Food Budget", "goal_type": "monthly", "target_amount": 500.00,
            "is_monthly": True,
        }
        response = client.post("/goals", json=payload)
        assert response.status_code == 201
    ...
    def test_update_goal(self, client: TestClient, db_session: Session):
        create_resp = client.post("/goals", json={
            "name": "Fund", "goal_type": "savings", "target_amount": 5000, "is_monthly": False,
        })
