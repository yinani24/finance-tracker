...
from dashboard.analytics import (
    ...
    compute_subscription_breakdown, compute_food_breakdown, compute_fixed_costs,
    compute_lifestyle_insights, compute_card_csp_analysis, compute_mom_changes,
)

TODAY = date(2026, 3, 15)  # fixed date for all tests

ACCOUNTS_FIXTURE = {"accounts": [{"name": "Chase-Checking", "type": "checking", "balance": 40000.0}]}
...
class TestHealthScore:
    ...
    def test_savings_rate_zero_yields_zero_savings_pts(self, sample_goals):
        ...
        result = compute_health_score(kpis, [], sample_goals, ACCOUNTS_FIXTURE, pd.DataFrame(), today=TODAY)
    ...
    def test_high_spending_trend_deducts_points(self, sample_goals):
        ...
        result = compute_health_score(kpis, trends, sample_goals, ACCOUNTS_FIXTURE, pd.DataFrame(), today=TODAY)
        assert isinstance(result["score"], int)
        assert 0 <= result["score"] <= 100
    ...
    def test_at_risk_goal_appended_to_failing(self):
        ...
        result = compute_health_score(kpis, [], goals, ACCOUNTS_FIXTURE, pd.DataFrame(), today=TODAY)
        assert isinstance(result["score"], int)
        assert 0 <= result["score"] <= 100
    ...
    def test_subscription_ratio_8_to_15_pct_yields_5_pts(self):
        ...
        result = compute_health_score(kpis, trends, {"goals": []}, ACCOUNTS_FIXTURE, pd.DataFrame(), today=TODAY)
        assert isinstance(result["score"], int)
        assert 0 <= result["score"] <= 100
...
class TestComputeMomChanges:
    ...
    def test_no_prior_month(self):
        rows = [
            {"date": "2026-03-01", "amount": 5000.0, "merchant": "Pay",
             "category": "Income", "account": "A", "source": "csv",
    ...
    def test_zero_prior_values(self):
        rows = [
            {"date": "2026-02-01", "amount": -0.01, "merchant": "X",
             "category": "Other", "account": "A", "source": "csv",
             "is_income": False, "is_savings": False, "notes": ""},
            {"date": "2026-03-01", "amount": -100.0, "merchant": "Y",
             "category": "Other", "account": "A", "source": "csv",
             "is_income": False, "is_savings": False, "notes": ""},
        ]
        df = pd.DataFrame(rows)
...
class TestComputeSubscriptionBreakdown:
    ...
    def test_groups_by_merchant_and_computes_monthly_avg(self):
        rows = [
            {"date": "2026-03-10", "amount": -15.99, "merchant": "Netflix", "category": "Subscriptions", "account": "A"},
            {"date": "2026-02-10", "amount": -15.99, "merchant": "Netflix", "category": "Subscriptions", "account": "A"},
            {"date": "2026-01-10", "amount": -15.99, "merchant": "Netflix", "category": "Subscriptions", "account": "A"},
        ]
    ...
    def test_detects_phone_redundancy(self):
        rows = [
            {"date": "2026-03-01", "amount": -25.00, "merchant": "visible 866-331-3527", "category": "Subscriptions", "account": "A"},
            {"date": "2026-03-01", "amount": -15.47, "merchant": "tmobile prepd autopy", "category": "Subscriptions", "account": "A"},
        ]
        result = compute_subscription_breakdown(self._make_df(rows), today=TODAY)
    ...
    def test_no_redundancy_with_single_phone_plan(self):
        rows = [{"date": "2026-03-01", "amount": -25.00, "merchant": "visible 866-331-3527", "category": "Subscriptions", "account": "A"}]
        result = compute_subscription_breakdown(self._make_df(rows), today=TODAY)
        assert result["redundancy_waste"] == 0.0

    def test_ignores_non_subscription_categories(self):
        rows = [
            {"date": "2026-03-01", "amount": -50.00, "merchant": "Amazon", "category": "Shopping", "account": "A"},
        ]
        result = compute_subscription_breakdown(self._make_df(rows), today=TODAY)
        assert result["services"] == []
...
class TestComputeFoodBreakdown:
    ...
    def test_groups_by_merchant(self):
        rows = [
            {"date": "2026-03-01", "amount": -12.50, "merchant": "Chipotle", "category": "Food & Dining", "account": "A"},
            {"date": "2026-03-05", "amount": -13.00, "merchant": "Chipotle", "category": "Food & Dining", "account": "A"},
        ]
    ...
    def test_no_food_transactions_returns_empty(self):
        rows = [{"date": "2026-03-01", "amount": -50.00, "merchant": "Uber", "category": "Transport", "account": "A"}]
...
class TestComputeFixedCosts:
    ...
    def test_detects_rent_from_housing_category(self):
        rows = [
            {"date": "2026-03-01", "amount": -1314.0, "merchant": "Rent", "category": "Housing", "account": "A"},
            {"date": "2026-02-01", "amount": -1314.0, "merchant": "Rent", "category": "Housing", "account": "A"},
            {"date": "2026-01-01", "amount": -1314.0, "merchant": "Rent", "category": "Housing", "account": "A"},
            {"date": "2026-03-15", "amount": 5000.0, "merchant": "Payroll", "category": "Income", "account": "A"},
            {"date": "2026-02-15", "amount": 5000.0, "merchant": "Payroll", "category": "Income", "account": "A"},
            {"date": "2026-01-15", "amount": 5000.0, "merchant": "Payroll", "category": "Income", "account": "A"},
        ]
    ...
    def test_pct_zero_when_no_income(self):
        rows = [{"date": "2026-03-01", "amount": -1000.0, "merchant": "Rent", "category": "Housing", "account": "A"}]
        result = compute_fixed_costs(self._make_df(rows), {"accounts": []}, today=TODAY)
        assert result["pct_of_income"] == 0.0
...
class TestComputeLifestyleInsights:
    def _empty_fixed(self):
        return {"rent": 0.0, "phone": 0.0, "insurance": 0.0, "subscriptions": 0.0, "total": 0.0, "avg_income": 0.0, "pct_of_income": 0.0}

    def _empty_sub(self):
        return {"services": [], "total_monthly": 0.0, "total_annual": 0.0, "redundancy_waste": 0.0}

    def test_fallback_when_no_rules_fire(self):
        result = compute_lifestyle_insights(EMPTY_DF, {}, self._empty_fixed(), self._empty_sub(), today=TODAY)
        assert len(result) >= 1
    ...
    def test_max_five_insights_returned(self):
        sub = {**self._empty_sub(), "redundancy_waste": 40.0, "total_monthly": 200.0, "total_annual": 2400.0}
        fixed = {**self._empty_fixed(), "rent": 2000.0, "avg_income": 3000.0}
        rows = [
            {"date": "2026-03-01", "amount": -800.0, "merchant": "Dubai Mall", "category": "Dubai", "account": "A"},
            {"date": "2026-02-01", "amount": -800.0, "merchant": "Dubai Mall", "category": "Dubai", "account": "A"},
            {"date": "2026-01-01", "amount": -800.0, "merchant": "Dubai Mall", "category": "Dubai", "account": "A"},
            {"date": "2026-03-01", "amount": -700.0, "merchant": "Chipotle", "category": "Food & Dining", "account": "A"},
            {"date": "2026-02-01", "amount": -700.0, "merchant": "Chipotle", "category": "Food & Dining", "account": "A"},
            {"date": "2026-01-01", "amount": -700.0, "merchant": "Chipotle", "category": "Food & Dining", "account": "A"},
        ]
        df = pd.DataFrame(rows)
        df["date"] = pd.to_datetime(df["date"])
...
class TestComputeHealthScoreNew:
    def _make_kpis(self, income=5000.0, expenses=4000.0):
        ...
        return {"income": income, "expenses": expenses, "saved": saved, "savings_rate": sr, "net_worth": 0.0, "monthly_target": 0.0, "this_month": "2026-03"}

    def test_returns_dimensions_list_of_six(self):
        kpis = self._make_kpis()
        result = compute_health_score(kpis, [], {"goals": []}, ACCOUNTS_FIXTURE, EMPTY_DF, today=TODAY)
...
class TestComputeCardCSPAnalysis:
    CSP = {
        ...
        "reward_type": "points", "points_cpp": 0.015,
        "rewards": {"Food & Dining": 3.0, "Subscriptions": 3.0, "Travel": 2.0, "Airlines": 2.0, "Other": 1.0}
    }
    ...
    def test_net_equals_gross_minus_fee(self):
        rows = [
            {"date": "2026-03-01", "amount": -100.0, "merchant": "Chipotle", "category": "Food & Dining", "account": "A"},
            {"date": "2026-02-01", "amount": -100.0, "merchant": "Chipotle", "category": "Food & Dining", "account": "A"},
            {"date": "2026-01-01", "amount": -100.0, "merchant": "Chipotle", "category": "Food & Dining", "account": "A"},
        ]
    ...
    def test_by_category_populated(self):
        rows = [
            {"date": "2026-03-01", "amount": -200.0, "merchant": "Chipotle", "category": "Food & Dining", "account": "A"},
            {"date": "2026-02-01", "amount": -200.0, "merchant": "Chipotle", "category": "Food & Dining", "account": "A"},
            {"date": "2026-01-01", "amount": -200.0, "merchant": "Chipotle", "category": "Food & Dining", "account": "A"},
        ]
        result = compute_card_csp_analysis(self._make_df(rows), self.CSP, today=TODAY)
    ...
    def test_airline_top_opportunity_triggers(self):
        ...
        rows = [
            {"date": "2026-03-01", "amount": -300.0, "merchant": "Delta", "category": "Airlines", "account": "A"},
            {"date": "2026-02-01", "amount": -300.0, "merchant": "Delta", "category": "Airlines", "account": "A"},
            {"date": "2026-01-01", "amount": -300.0, "merchant": "Delta", "category": "Airlines", "account": "A"},
        ]
        result = compute_card_csp_analysis(self._make_df(rows), self.CSP, today=TODAY)
    ...
    def test_1x_category_top_opportunity_triggers(self):
        ...
        rows = [
            {"date": "2026-03-01", "amount": -200.0, "merchant": "Target", "category": "Shopping", "account": "A"},
            {"date": "2026-02-01", "amount": -200.0, "merchant": "Target", "category": "Shopping", "account": "A"},
            {"date": "2026-01-01", "amount": -200.0, "merchant": "Target", "category": "Shopping", "account": "A"},
        ]
        result = compute_card_csp_analysis(self._make_df(rows), self.CSP, today=TODAY)
...
class TestCoverageNoneDefaults:
    ...
    def test_lifestyle_insights_defaults_today(self):
        fixed = {"rent": 0.0, "phone": 0.0, "insurance": 0.0, "subscriptions": 0.0, "total": 0.0, "avg_income": 0.0, "pct_of_income": 0.0}
    ...
    def test_card_csp_analysis_defaults_today(self):
        csp = {"name": "CSP", "annual_fee": 95, "reward_type": "points", "points_cpp": 0.015,
               "rewards": {"Food & Dining": 3.0, "Other": 1.0}}
        result = compute_card_csp_analysis(EMPTY_DF, csp)
        assert result["gross_rewards"] == 0.0

    def test_health_score_defaults_today(self):
        kpis = {"income": 5000.0, "expenses": 4000.0, "saved": 1000.0, "savings_rate": 0.2, "net_worth": 0.0, "monthly_target": 0.0, "this_month": "2026-03"}
        result = compute_health_score(kpis, [], {"goals": []}, ACCOUNTS_FIXTURE, EMPTY_DF)
        assert isinstance(result["score"], int)

    def test_get_at_risk_goals_defaults_today(self):
        goals = {"goals": [{"name": "Test Goal", "target_amount": 1000.0, "current_amount": 100.0,
                             "deadline": "2026-06", "created": "2026-01-01"}]}
...
class TestHealthScoreFixedCostBranches:
    ...
    def _make_kpis(self, income=5000.0, expenses=4000.0):
        saved = income - expenses
        sr = saved / income if income > 0 else 0.0
        return {"income": income, "expenses": expenses, "saved": saved, "savings_rate": sr,
                "net_worth": 0.0, "monthly_target": 0.0, "this_month": "2026-03"}
    ...
    def test_fixed_cost_ratio_warn_branch(self):
        ...
        rows = [
            {"date": "2026-03-01", "amount": -2250.0, "merchant": "Rent", "category": "Housing", "account": "A"},
            {"date": "2026-02-01", "amount": -2250.0, "merchant": "Rent", "category": "Housing", "account": "A"},
            {"date": "2026-01-01", "amount": -2250.0, "merchant": "Rent", "category": "Housing", "account": "A"},
            {"date": "2026-03-15", "amount": 5000.0, "merchant": "Payroll", "category": "Income", "account": "A"},
            {"date": "2026-02-15", "amount": 5000.0, "merchant": "Payroll", "category": "Income", "account": "A"},
            {"date": "2026-01-15", "amount": 5000.0, "merchant": "Payroll", "category": "Income", "account": "A"},
        ]
    ...
    def test_fixed_cost_ratio_fail_branch(self):
        ...
        rows = [
            {"date": "2026-03-01", "amount": -3000.0, "merchant": "Rent", "category": "Housing", "account": "A"},
            {"date": "2026-02-01", "amount": -3000.0, "merchant": "Rent", "category": "Housing", "account": "A"},
            {"date": "2026-01-01", "amount": -3000.0, "merchant": "Rent", "category": "Housing", "account": "A"},
            {"date": "2026-03-15", "amount": 5000.0, "merchant": "Payroll", "category": "Income", "account": "A"},
            {"date": "2026-02-15", "amount": 5000.0, "merchant": "Payroll", "category": "Income", "account": "A"},
            {"date": "2026-01-15", "amount": 5000.0, "merchant": "Payroll", "category": "Income", "account": "A"},
        ]
    ...
    def test_investment_rate_above_5pct(self):
        ...
        rows = [
            {"date": "2026-03-01", "amount": -300.0, "merchant": "Robinhood", "category": "Investments", "account": "A"},
            {"date": "2026-03-15", "amount": 5000.0, "merchant": "Payroll", "category": "Income", "account": "A"},
        ]
        df = self._make_df(rows)
        kpis = self._make_kpis()
    ...
    def test_investment_rate_below_5pct(self):
        ...
        rows = [
            {"date": "2026-03-01", "amount": -100.0, "merchant": "Robinhood", "category": "Investments", "account": "A"},
            {"date": "2026-03-15", "amount": 5000.0, "merchant": "Payroll", "category": "Income", "account": "A"},
        ]
        df = self._make_df(rows)
        kpis = self._make_kpis()
